"Cuando se necesitan crear colleciones de clases,
pero no se quiere especificar el tipo concreto de clase a usar,
ya que este puede variar de una situación a otra."
...
Si observais el test, vereis que en cada test se tienen que instanciar clases para cada
idioma, para cada conjunto de mensajes.

Queremos agrupar la decision de que idioma usar en una unica invocacion a un
abstract factory.

Cread este abstract factory, con una implementacions para "es" y otra para "en" y
despues adaptad el test para que use este factory.
